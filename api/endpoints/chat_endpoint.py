...
from services.llm_service import call_ollama, call_ollama_stream, strip_markdown
    ...
    with SessionLocal() as db:
        ...
        try:
            if intent == "count":
                ...
                rag_context = build_recent_context(
                    db, school_id=school_id, dept_id=dept_id,
                    count_context=count_context, intent=intent, source_ids=source_ids,
                )
            else:
                rag_context, query_embedding = await build_notice_context(
                    db, message=req.message, school_id=school_id, dept_id=dept_id,
                    query_type=query_type, count_context=count_context,
