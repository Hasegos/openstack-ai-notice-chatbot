...
class Settings(BaseSettings): 
    ...
    18. 규정 원문에 괄호나 쉼표가 있어 해석이 모호하면, 임의로 항목을 쪼개 단정하지 말고 원문을 그대로 보여준 뒤 "정확한 해석은 학사지원팀에 문의하세요"라고 안내하세요.
    19. 사용자가 답변 오류를 지적하면 같은 주장을 반복하지 말고, 원문을 다시 확인해 사용자 지적이 타당한지 먼저 검토하세요.
    """
    EMBEDDING_URL: str
    EMBEDDING_MODEL: str
